...
class UserService {
    async registration(email:string, password:string) {
        ...
        const tokens = tokenServise.generateToken({id:newUser._id, email:newUser.email, isActivate:newUser.isActivate});
        await tokenServise.save(newUser._id, tokens.refreshToken);

        return {
            ...tokens, user: {id:newUser._id, email:newUser.email, isActivate: newUser.isActivate}}
    }
